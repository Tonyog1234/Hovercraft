...
float voltage = 0;
float bar = 0;
const int trials = 50;  //measures 50 times, so we divide by 50 later. we can also do 100
int ADCvalue = 0;
int adc_avg = 0;
...
void MPU6050_read(int16_t* gx, int16_t* gy, int16_t* gyro_z) {
    ...
    I2C_write(0x43);                        //start reading at register 0x43 (gyroscope data), according to datasheet
    I2C_stop();                             //stop
...
void convert_raw_data() {
  ...
  //adding previous readings from gyro imitates somewhat of a numerical integration. provides readings combined over time.
  //0.25 is a compensation factor for the fact that the readings are not accurate at all. Without using filters, this is the simplest approach
  //trial and error gives readings that are decently accurate for usage.
  //during testing, range can go from 0.1 to 0.25
  // + 0.5 is depending on the reading. Through testing, values range from 0 to -70 (rarely -70, often -60)
  // 60/131 is =~ 0.45 so we counter this drift with +0.45 to counter the -a/g values reading.
  temp = (float)gyro_z / GYRO_SCALE * 0.12 + prev_gyro_z + 0.45;

  //using the temp values we get, 
  //using this method, our angle values will range from 0 to 180 only. This will be useful to determine CCW or CW turning.
  //if value printed shows 179 - > 90, we are turning CCW, while 0 -> 90 is turning CW
...
int analogRead_A0() {
  ...
  //return the value, it should give a value that is around 0 to 1023;1024;1025.
  return adc_value;
}
//------------------------------------ULTRASONIC Functions------------------------------------//
//uses 8-bit readings, which is worse than timer 1 but still decent.
void timer2_init() {
  TCCR2A = 0;
  TCCR2B = 0;
...
uint32_t get_timer_value() {
  ...
  overflows = timer_overflow_count;   //number of times it has overflows 256 ticks, when TCNT2 > 256.
...
void HCSR04_read() {
  ...
  while (!(PIND & (1 << Echo_pin))) {
    if (timer_overflow_count > 20) return;    //adjusted timeout for Timer2
  }
...
int top_hcsr04() {
  ...
  //this is going to be a method we can use for now to calculate timing between detections for bar
  //add duration while pulse is coming back (wait for reception)
  while (PIND & (1 << PD2)) {
    duration_top++;
  }
  //2MHZ so each duration (tick) is 0.5us, which we will just divide by 2 to compensate. 2 ticks per us basically.
...
void HCSR04_distance() {
  ...
  //because of our prescaler, we will multiply by 4. It also helps get better readings by testing.
...
void control_thrust_lift(int distance) {
  ...
  if (distance <= 15) {
    ...
    yaw = 0;                  //reset the yaw (so it does not cuase issues after turning) yaw is reset after each turn.
  }
  //wall not detected if under 15cm.
  //if over 42, keep going,
  //if it is in between, it will most likely re-do the algorithm and run into a wall.
  //if there is nothing in front, will always read over 42 cm.
  else if (distance > 42) {
    OCR0A = 250;
  }
}
...
void make_decision(int left, int right, int front) {
  ...
  if (front < 20) {
    if (left == max) {
      ...
      //turn until it is approx. 75 degrees, due to thrust not stopping instantly,
      //there is drift to compensate, which nearly turns 90 at the end of 75 yaw
      while (yaw < 75) {
        ...
        //failsafe, if after 5 seconds, the craft can't turn to 75 yaw, it will stop and go forward/check again
        if (stuck >= 5000) break;
    ...
    //backup in case of mis-read and glitches. if the front is max, but under 20, still go in front instead of random turns.
    //it will loop back to re-read and make a new decision after detecting the wall again. it will attempt to retry for a better turn.
    if (front == max) {
...
//The lift fan turns the hovercraft itself, but can be compensated if we know what degree.
void drifting_control() {
  OCR0A = 255;                                      //max speed for decent adjustments while moving forward
  OCR1A = OCR_map(yaw, -90, 90, OCR_min, OCR_max);  //correction by turning servo depending on the yaw IMU reading
  _delay_ms(100);                                   //delay so we don't move servo too fast.
}

//similar mapping function to arduino, using linear approximation.
long OCR_map(int16_t angle, long min_angle, long max_angle, long OCR_min, long OCR_max){
  return (angle - min_angle) * (OCR_max - OCR_min) / (max_angle - min_angle) + OCR_min;
}
//------------------------------------UART printing------------------------------------//
...
//--------------------------ALGORITHM for hovercraft starts here---------------------//
void setup() {
  UART_init();
  //Start lift fan.
  DDRD |= (1 << DDD6);
...
void loop() {
  ...
  for (int i = 0; i < trials; i++) {
    ...
    //we use 1023, 1024 and 1025 as we wish but we chose 1023 according to how others did it on tutorials
    //AVREF is using the external reference voltage which we set via the potentiometer 3.1 volts.
  ...
  if (bar < 20){
    ...
    PORTD &= ~(1<<PORTD4);
    PORTD &= ~(1<<PORTD7);
    stop = true;            //this prevents the loop from continuing on the next iteration.
  }  
